...
namespace Quest.MPDW.Customer
{
    public class SearchController : CustomerBaseController
    {
        ...
        public ActionResult Index(BaseUserSessionViewModel baseUserSessionViewModel)
        {
             ...
             * Return view.
             *---------------------------------------------------------------------------------------------------------------------------------*/
            CustomerSearchViewModel customerSearchViewModel = new CustomerSearchViewModel(this.UserSession, baseUserSessionViewModel);
            return View(customerSearchViewModel);
        }
        [HttpGet]
        public ActionResult List(BaseUserSessionViewModel baseUserSessionViewModel)
        {
            ...
            /*----------------------------------------------------------------------------------------------------------------------------------
             * Get list of Search for this user.
             *---------------------------------------------------------------------------------------------------------------------------------*/
            // TODO.
            CustomerSearchViewModel customerSearchViewModel = new CustomerSearchViewModel(this.UserSession);

            /*----------------------------------------------------------------------------------------------------------------------------------
             * Return view
             *---------------------------------------------------------------------------------------------------------------------------------*/
            status = new questStatus(Severity.Warning, "Not implemented yet");
            customerSearchViewModel.questStatus = status;
            return Json(customerSearchViewModel, JsonRequestBehavior.AllowGet);
        }
