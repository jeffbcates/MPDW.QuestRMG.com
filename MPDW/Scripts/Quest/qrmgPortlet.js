...
function qrmgPortlet(model) {
    ...
    _self._dooper = function (o) {
        qrmgmvc.Global.Maskit($(_self._e).find('.portlet'));
        if (o.validate) {
            if (_self.Validate()) {
                qrmgmvc.Global.Unmask($(_self._e).find('.portlet'));
                return;
            }
        }
        var _d = _self.GetData();
    ...
    _self._render = function () {
        var _e = $(_self._e);
        if (!_e) { alert('csPortlet element ' + _self._e + ' not found!'); return; }
    ...
    _self._rndrtt = function () {
        ...
        var _h = [], _i = 0;
        $.each(_self._model.actions, function (i, a) {
            if (a.type && a.type == 'tab') {
                _h[_i++] = '<div id="' + (_self._pfx + a.name + 'Tab') + '" class="portlet-tab hidden">';
                _h[_i++] = '<p>THIS IS TAB ' + a.name + '</p>';
                _h[_i++] = '</div>';
            }
        });
        $.each(_self._model.operations, function (i, o) {
            if (o.type && o.type == 'tab') {
                _h[_i++] = '<div id="' + (_self._pfx + o.name + 'Tab') + '" class="portlet-tab hidden">';
                _h[_i++] = '<p>THIS IS TAB ' + o.name + '</p>';
                _h[_i++] = '</div>';
            }
        });
        return (_h.join(''));
    ...
    _self._read = function (Id) {
        var _csio = new qrmgio(_self._rread, _self._model);
        var _d = { Id: Id };
        if (_self._model.ctx) {
    ...
    _self.LoadOptions = function (f) {
        ...
        var _csio = new qrmgio(_self._ropt, f);
        _csio._ajax('get', 'json', _url, _d);
    }
