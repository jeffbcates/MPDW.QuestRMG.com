    ...
    public class DbHTTPRequestLogsMgr : DbMgrSessionBased
    {
        ...
        public questStatus Clear()
        {
            try
            {
                using (MasterPricingEntities dbContext = new MasterPricingEntities())
                {
                    dbContext.HTTPRequestLogs.RemoveRange(dbContext.HTTPRequestLogs.Where(r => r.Id > 0));
                    dbContext.SaveChanges();
                }
            }
            catch (System.Exception ex)
            {
                return (new questStatus(Severity.Fatal, String.Format("EXCEPTION: clearing HTTP Requests Log {0}", ex.Message)));
            }
            return (new questStatus(Severity.Success));
        }
        #endregion
