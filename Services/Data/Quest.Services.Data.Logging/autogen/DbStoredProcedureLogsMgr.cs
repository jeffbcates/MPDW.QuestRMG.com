    ...
    public class DbStoredProcedureLogsMgr : DbMgrSessionBased
    {
        ...
        public questStatus Clear()
        {
            try
            {
                using (MasterPricingEntities dbContext = new MasterPricingEntities())
                {
                    dbContext.StoredProcedureLogs.RemoveRange(dbContext.StoredProcedureLogs.Where(r => r.Id > 0));
                    dbContext.SaveChanges();
                }
            }
            catch (System.Exception ex)
            {
                return (new questStatus(Severity.Fatal, String.Format("EXCEPTION: clearing Stored Procedures Log {0}", ex.Message)));
            }
            return (new questStatus(Severity.Success));
        }
        #endregion
